...
def handle_undo(self, arguments):
    ...
    if len(self.current_deltas) == 0:
      return
    # Find the index of the last 'role': 'user' entry
    last_user_index = None
    for i, message in enumerate(self.current_deltas):
    ...
    if last_user_index is not None:
        removed_messages = self.current_deltas[last_user_index:]
        self.current_deltas = self.current_deltas[:last_user_index]
...
def handle_save_message(self, json_path):
    ...
    with open(json_path, 'w') as f:
      json.dump(self.current_deltas, f, indent=2)

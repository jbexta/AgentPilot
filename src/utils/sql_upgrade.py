...
class SQLUpgrade:
    ...
    def v0_2_0(self):
        ...
        app_config = """{"system.language": "English", "system.dev_mode": false, "system.always_on_top": true, "system.auto_title": true, "system.auto_title_model": "claude-3-sonnet-20240229", "system.auto_title_prompt": "Write only a brief and concise title for a chat that begins with the following message:\n\n```{user_msg}```", "system.voice_input_method": "None", "display.primary_color": "#262326", "display.secondary_color": "#3f3a3f", "display.text_color": "#c1b5d5", "display.text_font": "Droid Sans Fallback", "display.text_size": 15, "display.show_bubble_name": "In Group", "display.show_bubble_avatar": "Always", "display.bubble_avatar_position": "Top", "display.bubble_spacing": 7}"""
        sql.execute("""
        INSERT INTO settings (field, value) VALUES
            ('app_config', ?)""", (app_config,))

        default_agent = """{"info.name": "Assistant", "info.avatar_path": "", "info.use_plugin": "", "context.model": "mistral/mistral-medium", "context.display_markdown": true, "context.sys_msg": "", "context.max_messages": 15, "context.max_turns": 10, "context.on_consecutive_response": "REPLACE", "context.user_msg": "", "chat.preload.data": "[]", "group.hide_responses": false, "group.output_context_placeholder": "", "group.on_multiple_inputs": "Use system message", "group.set_members_as_user_role": true, "files.data": "[]", "tools.data": "[]"}"""
        sql.execute("""

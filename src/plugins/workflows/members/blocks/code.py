...
@set_module_type(module_type='Members', plugin='BLOCK', settings='code_settings')
class Code(Block):
    default_role = 'block'
    ...
    async def receive(self):
        ...
        environment_tup = system.manager.environments.get(env_id)
        if environment_tup is None:
            environment_tup = system.manager.environments.get_env_from_name('Local')
        if environment_tup is None:
        ...
        # venv = system.manager.venvs.get(venv_name)

        # if venv_name == 'default' or not venv or not venv.has_package('ipykernel'):

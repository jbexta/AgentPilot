...
def setup_text_llm(interpreter):
    ...
    if interpreter.local:
        ...
        # Download HF models
        if openinterpreter.model.startswith("huggingface/"):
            ...
            return setup_local_text_llm(openinterpreter)
        
        # If we're here, it means the user wants to use
        # an OpenAI compatible endpoint running on localhost

        if openinterpreter.api_base is None:
            raise Exception('''To use Open Interpreter locally, either provide a huggingface model via `openinterpreter --model huggingface/{huggingface repo name}`
                            or a localhost URL that exposes an OpenAI compatible endpoint by setting `openinterpreter --api_base {localhost URL}`.''')
        
        # Tell LiteLLM to treat the endpoint as an OpenAI proxy
        model = "custom_openai/" + openinterpreter.model
        ...
        try:
            ...
            raise Exception("Architecture not yet supported for local LLM inference. Please run `openinterpreter` to connect to a cloud model, then try `--local` again in a few days.")

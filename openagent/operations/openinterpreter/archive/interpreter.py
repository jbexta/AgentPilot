...
# **Open Interpreter** will require approval before running code. Use `openinterpreter -y` to bypass this.
#
...
#             welcome_message += f"\n> Model set to `{notice_model}`\n\n**Tip:** To run locally, use `openinterpreter --local`"
#
#         if self.local:
...
#
#                             arguments = {"code": code}
#                             if language:  # We only add this if we have it-- the second we have it, an openinterpreter gets fired up (I think? maybe I'm wrong)
#                                 if language == "bash":
#                                     language = "shell"

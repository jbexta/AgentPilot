...
#
# At the same time, I plan on pushing a significant re-factor of `openinterpreter.py` and `code_interpreter.py` ~ September 21st.
#
...
#     current_version = pkg_resources.get_distribution("open-openinterpreter").version
#
#     return version.parse(latest_version) > version.parse(current_version)
#
# def cli(interpreter):
#   """
#   Takes an instance of openinterpreter.
#   Modifies it according to command line flags, then runs chat.
#   """
#
#   try:
#     if check_for_update():
#       print("A new version is available. Please run 'pip install --upgrade open-openinterpreter'.")
#   except:
#     # Fine if this fails
#     pass
#
...
#
#   if args.version:
#     print("Open Interpreter", pkg_resources.get_distribution("open-openinterpreter").version)
#     return
#
#   if args.max_tokens:
...
#   # check if user passed in a max budget (USD) for their LLM API Calls (e.g. `openinterpreter --max_budget 0.001` # sets a max API call budget of $0.01) - for more: https://docs.litellm.ai/docs/budget_manager
#   litellm.max_budget = (args.max_budget or os.getenv("LITELLM_MAX_BUDGET"))
#   # Modify openinterpreter according to command line flags
#   if args.yes:
...
#     # This way, when folks hit openinterpreter --local, they get the same experience as before.
#
...
#
#     # Temporarily, for backwards (behavioral) compatability, we've moved this part of llama_2.py here.
#     # This way, when folks hit openinterpreter --falcon, they get the same experience as --local.
#

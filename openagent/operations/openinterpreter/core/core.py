...
class Interpreter:
    ...
    def __init__(self):
        ...
        #     if check_for_update():
        #         display_markdown_message("> **A new version of Open Interpreter is available.**\n>Please run: `pip install --upgrade open-openinterpreter`\n\n---")

    def chat(self, message=None, display=True, stream=False):
        if stream:
            return self.streaming_chat(message=message, display=display)

        # If stream=False, *pull* from the stream.
        for _ in self.streaming_chat(message=message, display=display):
            pass

        return self.messages

    # def hook_chat(self, message=None, display=True, stream=False):
    #     if stream:
    ...
    def streaming_chat(self, message=None, display=True):
        ...
        raise Exception("`openinterpreter.chat()` requires a display. Set `display=True` or pass a message into `openinterpreter.chat(message)`.")
